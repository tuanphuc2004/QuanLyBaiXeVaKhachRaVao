...
def get_visitors(
    ...
    current_role: Role = Depends(require_roles(Role.admin, Role.employee, Role.director)),
):
    logger.info(
...
def get_pending_approval_count(
    current_role: Role = Depends(require_roles(Role.admin, Role.director)),
):
...
def approve_visitor(
    ...
    """
    Admin hoặc Giám đốc xác nhận lịch hẹn cho khách được tạo tại quầy lễ tân.
